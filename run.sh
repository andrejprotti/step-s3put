...
then
    ...
    sudo apt-get -y update && sudo apt-get -y install s3cmd
...
set +e
debug "s3cmd put --verbose '$WERCKER_S3PUT_FILE' '$WERCKER_S3PUT_URL'"
sync_output=$(s3cmd sync --verbose "$WERCKER_S3PUT_FILE" "$WERCKER_S3PUT_URL")
